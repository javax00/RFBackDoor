...
now = datetime.now()
dt_string = now.strftime("%b-%d-%Y_%H-%M-%S")
filepathHeader = '/Users/'+username+'/Work/RFBackDoor/Pasternack_'+str(dt_string)+'.csv'
...
df = pd.read_csv('pasternack_links.csv')
...
driver.get('https://www.pasternack.com/')
					...
					for x in site_json['aaData']:
						...
						for i in x:
							if str(i) == 'Select':
								...
								c += 1
								print(str(c)+'. '+sku)
								fList.extend([manufac, cat, subCat, sku, 'https://www.pasternack.com/images/Product/large/'+sku+'.jpg', 'https://www.pasternack.com/images/productPDF/'+sku+'.pdf'])
							if str(i) == 'Sku':
								sku = str(x[i]).split('"')[3]
								fList.append('https://www.pasternack.com'+sku)
						...
						for x in fList[7:]:
							if x != '':
								if fList[3:] not in dupes:
									append_list_as_row(filepathHeader, fList)
									dupes.append(fList[3:])
								break
